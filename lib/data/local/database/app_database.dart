...
class Tasks extends Table {
  ...
  TextColumn get taskType => text()(); // watering, fertilizing, harvesting, etc.
...
class AppDatabase extends _$AppDatabase {
  ...
  Future<Field?> getFieldById(String fieldId) async {
    return (select(fields)..where((tbl) => tbl.id.equals(fieldId))).getSingleOrNull();
  }
  ...
  Future<int> deleteField(String id) async {
    return (update(fields)..where((tbl) => tbl.id.equals(id)))
        .write(const FieldsCompanion(isDeleted: Value(true), isSynced: Value(false)));
  }
  
  Future<List<Field>> getUnsyncedFields(String userId) async {
    return (select(fields)
      ..where((tbl) => tbl.userId.equals(userId) & tbl.isSynced.equals(false)))
        .get();
  }
  
  // ==================== TASK OPERATIONS ====================
  
  Stream<List<Task>> watchTasksByUserId(String userId, {DateTime? startDate, DateTime? endDate}) {
    var query = select(tasks)..where((tbl) => tbl.userId.equals(userId) & tbl.isDeleted.equals(false));
    
    if (startDate != null && endDate != null) {
      query = query..where((tbl) => tbl.dueDate.isBetweenValues(startDate, endDate));
    }
    
    return (query..orderBy([(tbl) => OrderingTerm(expression: tbl.dueDate)])).watch();
  }
  ...
  Future<List<DiaryEntry>> getDiaryEntriesByCategory(String userId, String category) async {
    return (select(diaryEntries)
      ..where((tbl) => 
        tbl.userId.equals(userId) & 
  ...
  Future<DiaryEntry?> getDiaryEntryById(String entryId) async {
    return (select(diaryEntries)..where((tbl) => tbl.id.equals(entryId))).getSingleOrNull();
  }
  ...
  Future<int> deleteDiaryEntry(String id) async {
    return (update(diaryEntries)..where((tbl) => tbl.id.equals(id)))
        .write(const DiaryEntriesCompanion(isDeleted: Value(true), isSynced: Value(false)));
  }
  ...
  Stream<List<MarketPrice>> watchMarketPrices() {
    return (select(marketPrices)
      ..orderBy([(tbl) => OrderingTerm(expression: tbl.priceDate, mode: OrderingMode.desc)]))
        .watch();
  }
  ...
  Future<Post?> getPostById(String postId) async {
    return (select(posts)..where((tbl) => tbl.id.equals(postId))).getSingleOrNull();
  }
  ...
  Future<int> deletePost(String id) async {
    return (update(posts)..where((tbl) => tbl.id.equals(id)))
        .write(const PostsCompanion(isDeleted: Value(true), isSynced: Value(false)));
  }
  ...
  Stream<List<Comment>> watchCommentsByPostId(String postId) {
    return (select(comments)
      ..where((tbl) => tbl.postId.equals(postId) & tbl.isDeleted.equals(false))
  ...
  Future<Comment?> getCommentById(String commentId) async {
    return (select(comments)..where((tbl) => tbl.id.equals(commentId))).getSingleOrNull();
  }
  ...
  Future<int> deleteComment(String id) async {
    return (update(comments)..where((tbl) => tbl.id.equals(id)))
        .write(const CommentsCompanion(isDeleted: Value(true), isSynced: Value(false)));
  }
  
  // ==================== SCHEMES OPERATIONS ====================
  
  Future<List<Scheme>> getSchemesByLanguage(String language) async {
    return (select(schemes)..where((tbl) => tbl.language.equals(language))).get();
  }
  ...
  Future<Scheme?> getSchemeById(String schemeId) async {
    return (select(schemes)..where((tbl) => tbl.id.equals(schemeId))).getSingleOrNull();
  }
  ...
  Future<int> deleteChatMessagesByUserId(String userId) async {
    return (delete(chatMessages)..where((tbl) => tbl.userId.equals(userId))).go();
  }
  ...
  Future<List<DiaryEntry>> getUnsyncedDiaryEntries() async {
    return (select(diaryEntries)..where((tbl) => tbl.isSynced.equals(false))).get();
  }
