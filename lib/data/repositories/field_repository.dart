...
class FieldRepository {
  ...
  Stream<Either<Failure, List<db.Field>>> watchFieldsByUserId(String userId) {
    try {
      return _database.watchFieldsByUserId(userId).map((fields) => Right(fields));
    } catch (e) {
  ...
  Future<Either<Failure, List<db.Field>>> getFieldsByUserId(String userId) async {
    try {
  ...
  Future<Either<Failure, int>> createField({
    ...
    try {
      ...
      if (coordinates.length < 3) {
        return const Left(ValidationFailure(message: 'Field must have at least 3 coordinate points'));
      }
  ...
  Future<Either<Failure, bool>> updateField({
    ...
    try {
      ...
      final field = db.FieldsCompanion(
        ...
        cropType: cropType != null ? drift.Value(cropType) : const drift.Value.absent(),
        soilType: soilType != null ? drift.Value(soilType) : const drift.Value.absent(),
        updatedAt: drift.Value(now),
        isSynced: const drift.Value(false),
      );
  ...
  /// Sync field to Firestore (background operation)
  void _syncFieldToCloud(String fieldId) async {
    try {
      final field = await _database.getFieldById(fieldId);
      if (field == null) return;

      await _firestore.collection('fields').doc(fieldId).set({
        'userId': field.userId,
        'name': field.name,
      ...
      AppLogger.info('Field synced to cloud: $fieldId');
    } catch (e) {
      AppLogger.error('Error syncing field to cloud', e);
    }
  }

  /// Sync all unsynced fields
  Future<void> syncAllFields(String userId) async {
    try {
      final fields = await _database.getUnsyncedFields(userId);
      for (final field in fields) {
        _syncFieldToCloud(field.id);
      }
      AppLogger.info('Synced ${fields.length} fields to cloud');
    } catch (e) {
      AppLogger.error('Error syncing all fields', e);
    }
  }
}
