...
class SchemesRepository {
  ...
  Future<Either<Failure, List<db.Scheme>>> getAllSchemes({String? language}) async {
    try {
      final schemes = await _database.getAllSchemes(language: language);
      
      // If no cached schemes or cache is old, fetch from Firestore
      if (schemes.isEmpty || _isCacheExpired(schemes)) {
        await _fetchSchemesFromCloud(language);
        final updatedSchemes = await _database.getAllSchemes(language: language);
        return Right(updatedSchemes);
      }
  ...
  Future<Either<Failure, bool>> refreshSchemes({String? language}) async {
    try {
      await _fetchSchemesFromCloud(language);
      AppLogger.info('Schemes refreshed from cloud');
  ...
  Future<void> _fetchSchemesFromCloud(String? language) async {
    try {
      Query query = _firestore.collection('schemes');
      ...
      for (final doc in snapshot.docs) {
        ...
        final scheme = db.SchemesCompanion(
          ...
          eligibilityCriteria: drift.Value(jsonEncode(data['eligibilityCriteria'] ?? {})),
          benefits: drift.Value(data['benefits'] ?? ''),
          applyUrl: drift.Value(data['applyUrl']),
          language: drift.Value(data['language'] ?? 'en'),
      ...
      AppLogger.info('Fetched ${snapshot.docs.length} schemes from cloud');
    } on FirebaseException catch (e) {
      AppLogger.error('Firestore error fetching schemes', e);
      throw NetworkException(message: e.message ?? 'Failed to fetch schemes');
    } catch (e) {
      AppLogger.error('Error fetching schemes from cloud', e);
      rethrow;
    }
  }
  ...
  bool _checkEligibility(db.Scheme scheme, Map<String, dynamic> userContext) {
    try {
      final criteria = jsonDecode(scheme.eligibilityCriteria) as Map<String, dynamic>;
      ...
      if (criteria['cropTypes'] != null) {
        final requiredCrops = (criteria['cropTypes'] as List).map((e) => e.toString()).toList();
      ...
      if (criteria['states'] != null) {
        final allowedStates = (criteria['states'] as List).map((e) => e.toString()).toList();
    ...
    try {
      final sampleSchemes = [
        {
          'id': 'pm_kisan',
          'title': 'PM-KISAN',
          'description': 'Pradhan Mantri Kisan Samman Nidhi - Direct income support of ₹6,000 per year',
      ...
      for (final schemeData in sampleSchemes) {
        final scheme = db.SchemesCompanion(
          ...
          eligibilityCriteria: drift.Value(schemeData['eligibilityCriteria'] as String),
          benefits: drift.Value(schemeData['benefits'] as String),
          applyUrl: drift.Value(schemeData['applyUrl'] as String?),
